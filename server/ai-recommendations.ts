...
export class AIRecommendationEngine {
  private getSpiritualContext(context: RecommendationContext): string {
    ...
    if (favoriteSongs && favoriteSongs.length > 0) {
      const favoriteAuthors = Array.from(new Set(favoriteSongs.map(song => song.author))).join(', ');
      const favoriteCategories = Array.from(new Set(favoriteSongs.map(song => song.category))).join(', ');
  ...
  async generateRecommendations(
    ...
    try {
      ...
      const songList = availableSongs.map(song => {
        return `${song.title} by ${song.author} (${song.category}, ${song.mood})`;
      }).join('\n');
...
Respond in JSON format:
{
  ...
  private getFallbackRecommendations(
    ...
    if (context.favoriteSongs && context.favoriteSongs.length > 0) {
      const favoriteAuthors = Array.from(new Set(context.favoriteSongs.map(s => s.author)));
    ...
    if (recommendations.length < count) {
      const remaining = availableSongs.filter(song =>
  ...
  async analyzeUserPreferences(
    ...
    if (!process.env.OPENAI_API_KEY) {
      const preferredAuthors = Array.from(new Set(favoriteSongs.map(s => s.author)));
      const preferredMoods = Array.from(new Set(favoriteSongs.map(s => s.mood)));
